...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        if state in walls or state[0] < 0 or state[0] > n-1 or state[1] < 0 or state[1] > n - 1:
            return False
        return True
    ...
    def successor(self, state):
        ...
        if self.check_valid((x_pos, y_pos + 1), self.walls, self.n):
            successors[f'Gore 1'] = (x_pos, y_pos + 1)

        if self.check_valid((x_pos, y_pos + 2), self.walls, self.n):
            successors[f'Gore 2'] = (x_pos, y_pos + 2)

        if self.check_valid((x_pos, y_pos + 3), self.walls, self.n):
            successors[f'Gore 3'] = (x_pos, y_pos + 3)


        if self.check_valid((x_pos + 1, y_pos), self.walls, self.n):
            successors[f'Desno 1'] = (x_pos + 1, y_pos)

        if self.check_valid((x_pos + 2, y_pos), self.walls, self.n):
            successors[f'Desno 2'] = (x_pos + 2, y_pos)

        if self.check_valid((x_pos + 3, y_pos), self.walls, self.n):
            successors[f'Desno 3'] = (x_pos + 3, y_pos)
