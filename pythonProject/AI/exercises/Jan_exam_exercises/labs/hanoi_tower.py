...
class Hanoi(Problem):
    def __init__(self, initial, goal=None):
        super(Hanoi, self).__init__(initial, goal)
    ...
    def successor(self, state):
        ...
        # ['3', ',', '2', ',', '1', ';', ';']
        array = ''.join([i for i in line])
        array_splitted = array.split(";")

        for i in range(len(array_splitted)):
            if len(array_splitted[i]) > 0:
                get_last = array_splitted[i].__getitem__(-1)
                temp = []
...
if __name__ == "__main__":
    ...
    for i in range(len(line)):
        reverse.append(line[len(line)-1-i])

    final = ''.join([i for i in reverse])


    hanoi = Hanoi(tuple(line), tuple(final))
