...
class Squares(Problem):
    def __init__(self, initial, goal=None):
        super(Squares, self).__init__(initial, goal)
    ...
    def successor(self, state):
        ...
        for i in range(len(squares)):
            ...
            # up
            new_state = (chosen[0], chosen[1] + 1)
            if Squares.check_valid(new_state):
                # new_list.insert(i, new_state)
                new_list[i] = new_state
                succ[f"Pomesti kvadratche {i + 1} gore"] = tuple(new_list)
            ...
            if Squares.check_valid(new_state):
                # new_list.insert(i, new_state)
                new_list[i] = new_state
                succ[f"Pomesti kvadratche {i + 1} dolu"] = tuple(new_list)

            # left
            new_state = (chosen[0] - 1, chosen[1])
            if Squares.check_valid(new_state):
                # new_list.insert(i, new_state)
                new_list[i] = new_state
                succ[f"Pomesti kvadratche {i + 1} levo"] = tuple(new_list)
            # right
            new_state = (chosen[0]+1, chosen[1])
            if Squares.check_valid(new_state):
                # new_list.insert(i,new_state)
                new_list[i] = new_state
                succ[f"Pomesti kvadratche {i+1} desno"] = tuple(new_list)
