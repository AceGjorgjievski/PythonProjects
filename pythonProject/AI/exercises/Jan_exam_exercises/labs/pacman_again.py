...
class Pacman(Problem):
    ...
    def successor(self, state):
        ...
        # (px, py, dir, tuple(dots))

        if dir == "istok":
            #right
            new_state = (px+1, py)
            if px+1 < 10 and new_state not in self.obstacles:
                succ["ProdolzhiPravo"] = (px+1, py, "istok",
                                          tuple([t for t in dots if new_state != t]))
            # left
            new_state = (px-1, py)
            if px-1 >=0 and new_state not in self.obstacles:
                succ["ProdolzhiNazad"] = (px-1, py, "zapad",
                                          tuple([t for t in dots if new_state != t]))

            # up
            new_state = (px, py+1)
            if py+1 < 10 and new_state not in self.obstacles:
                succ["SvrtiLevo"] = (px, py+1, "sever",
                                     tuple([t for t in dots if new_state != t]))

            # down
            new_state = (px, py-1)
            if py-1 >=0 and new_state not in self.obstacles:
                succ["SvrtiDesno"] = (px, py-1, "jug",
                                      tuple([t for t in dots if new_state != t]))
        ...
        if dir == "zapad":
            ...
            new_state = (px+1, py)
            if px+1 < 10 and new_state not in self.obstacles:
                succ["ProdolzhiNazad"] = (px+1, py, "istok",
                                          tuple([t for t in dots if new_state != t]))
            # left
            new_state = (px-1, py)
            if px-1 >= 0 and new_state not in self.obstacles:
                succ["ProdolzhiPravo"] = (px-1, py, "zapad",
                                          tuple([t for t in dots if new_state != t]))

            #up
            new_state = (px, py+1)
            if py+1 < 10 and new_state not in self.obstacles:
                succ["SvrtiDesno"] = (px, py+1, "sever",
                                      tuple([t for t in dots if new_state != t]))

            #down
            new_state = (px, py-1)
            if py-1 >= 0 and new_state not in self.obstacles:
                succ["SvrtiLevo"] = (px, py-1, "jug",
                                     tuple([t for t in dots if new_state != t]))
        ...
        if dir == "sever":
            ...
            new_state = (px+1, py)
            if px+1 < 10 and new_state not in self.obstacles:
                succ["SvrtiDesno"] = (px+1, py, "istok",
                                      tuple([t for t in dots if new_state != t]))

            #left
            new_state = (px-1, py)
            if px-1 >= 0 and new_state not in self.obstacles:
                succ["SvrtiLevo"] = (px-1, py, "zapad",
                                     tuple([t for t in dots if new_state != t]))

            #up
            new_state = (px, py+1)
            if py+1 < 10 and new_state not in self.obstacles:
                succ["ProdolzhiPravo"] = (px, py+1, "sever",
                                          tuple([t for t in dots if new_state != t]))

            #down
            new_state = (px, py-1)
            if py-1 >= 0 and new_state not in self.obstacles:
                succ["ProdolzhiNazad"] = (px, py-1, "jug",
                                          tuple([t for t in dots if new_state != t]))
        ...
        if dir == "jug":
            ...
            new_state = (px+1, py)
            if px+1 < 10 and new_state not in self.obstacles:
                succ["SvrtiLevo"] = (px+1, py, "istok",
                                     tuple([t for t in dots if new_state != t]))
            #left
            new_state = (px-1, py)
            if px-1>=0 and new_state not in self.obstacles:
                succ["SvrtiDesno"] = (px-1, py, "zapad",
                                      tuple([t for t in dots if new_state != t]))
            #up
            new_state = (px, py+1)
            if py+1 < 10 and new_state not in self.obstacles:
                succ["ProdolzhiNazad"] = (px, py+1, "sever",
                                          tuple([t for t in dots if new_state != t]))
            #down
            new_state = (px, py-1)
            if py-1 >= 0 and new_state not in self.obstacles:
                succ["ProdolzhiPravo"] = (px, py-1, "jug",
                                          tuple([t for t in dots if new_state != t]))
