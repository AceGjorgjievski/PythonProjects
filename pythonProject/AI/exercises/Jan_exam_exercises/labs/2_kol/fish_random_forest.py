...
if __name__ == '__main__':
    ...
    train_set = dataset2[:int(0.85*len(dataset2))]
    train_x = [row[:-1] for row in train_set]
    train_y = [row[-1] for row in train_set]

    test_set = dataset2[int(0.85*len(dataset2)):]
    ...
    classifier = RandomForestClassifier(n_estimators=num_trees, criterion=criteria, random_state=0)
    ...
    for i in range(len(entry)):
       if i != col_index:
              transformed_entry.append(entry[i])



    predicted_entry = classifier.predict([transformed_entry])[0]

    entry_probabilities = classifier.predict_proba([transformed_entry])[0]

    print(f"Accuracy: {accuracy}")
    print(f"{predicted_entry}")
    print(f"{entry_probabilities}")
