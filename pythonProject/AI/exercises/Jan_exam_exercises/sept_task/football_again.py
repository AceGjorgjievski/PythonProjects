...
class FootBall(Problem):
    def __init__(self, initial, obstacles, goal, grid_size):
    ...
    def check_valid(state):
        ...
        if x < 0 and x >= 7 and y < 0 and y >= 6:
            return False
        return True
    ...
    def successor(self, state):
        ...
        # (tuple(man_pos), tuple(ball_pos))

        # ball - left
        new_man_pos = (mx-1, my)
        new_ball_pos = (bx-1, by)
        if FootBall.check_valid(new_ball_pos) and FootBall.check_valid(new_man_pos) and \
            new_man_pos not in self.obstacles and new_ball_pos not in self.obstacles and \
                (bx+1,by) == (mx, my):
        ...
        #man - left
        new_man_pos = (mx-1, my)
        if FootBall.check_valid(new_man_pos) and new_man_pos not in self.obstacles:
    ...
    def goal_test(self, state):
        # (tuple(man_pos), tuple(ball_pos))
        bx, by = state[1]
        if (bx,by) == self.goal[0] or (bx, by) == self.goal[1]:
            return True
        return False
