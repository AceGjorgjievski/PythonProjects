...
class Disks(Problem):
    def __init__(self, initial, num_disks, length, goal=None):
    ...
    def successor(self, state):
        ...
        for i in range(len(state)):
            ...
            # right - 1
            if i+1 < self.length and lista[i+1] == 0:
                new_array = lista
                new_array[i+1] = new_array[i]
                new_array[i] = 0
                succ[f"D1: Disk {lista[i]}"] = tuple(new_array)

            # right - 2
            if i+2 < self.length and lista[i+1] != 0 and lista[i+2] == 0:
                new_array = lista
                new_array[i+2] = new_array[i]
                new_array[i] = 0
                succ[f"D2: Disk {lista[i]}"] = tuple(new_array)
            ...
            if i > 0 and lista[i-1] == 0:
                ...
                new_array[i-1] = new_array[i]
                new_array[i] = 0
                succ[f"L1: Disk {lista[i]}"] = tuple(new_array)

            # left - 2
            if i > 1 and lista[i-1] != 0 and lista[i-2] == 0:
                new_array = lista
                new_array[i-2] = new_array[i]
                new_array[i] = 0
                succ[f"L2: Disk {lista[i]}"] = tuple(new_array)
...
if __name__ == "__main__":
    ...
    for i in range(len(initial),length):
        initial.append(0)
        goal.append(0)

    goal.reverse()

    disks = Disks(tuple(initial), num_disks, length, tuple(goal))

    result = astar_search(disks).solution()
