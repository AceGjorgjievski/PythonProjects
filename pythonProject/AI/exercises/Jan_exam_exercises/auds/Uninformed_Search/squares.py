...
class Squares(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(len(squares)):
            ...
            new_state = (x, y+1)
            temp[i] = new_state
            if Squares.check_valid(temp):
                succ[f"Pomesti kvadratche {i+1} gore"] = tuple(temp)

            #down
            new_state = (x, y-1)
            temp[i] = new_state
            if Squares.check_valid(temp):
                succ[f"Pomesti kvadratche {i+1} dolu"] = tuple(temp)

            # left
            new_state = (x-1, y)
            temp[i] = new_state
            if Squares.check_valid(temp):
                succ[f"Pomesti kvadratche {i+1} levo"] = tuple(temp)

            # right
            new_state = (x+1, y)
            temp[i] = new_state
            if Squares.check_valid(temp):
                succ[f"Pomesti kvadratche {i+1} desno"] = tuple(temp)
