...
def get_data():
    return [[180.0, 23.6, 25.2, 27.9, 25.4, 14.0, 'Roach'],
...
if __name__ == '__main__':
    ...
    classifier = RandomForestClassifier(n_estimators=number_trees,criterion=criterion_input, random_state=0)
    ...
    classifier.fit(train_x2, train_y)

    accuracy = 0
    for i in range(len(test_x2)):
       predicted_class = classifier.predict([test_x2[i]])[0]
       true_class = test_y[i]
    ...
    print(f"Accuracy: {accuracy/len(test_set2)}")

    # input_2 = []
    # for i in range(len(input)):
    #     if i != col_index:
    #         input_2.append(input[i])

    input_2 = [input[i] for i in range(len(input)) if i != col_index]

    predicted_input = classifier.predict([input_2])[0]
    print(f"{predicted_input}")


    probability = classifier.predict_proba([input_2])[0]
    print(f"{probability}")
