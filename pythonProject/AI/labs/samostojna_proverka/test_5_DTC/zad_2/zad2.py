...
if __name__ == '__main__':
    ...
    num_trees = int(input())
    criterion = input()
    to_classifie = [int(el) for el in input().split(" ")]
    ...
    for i in range(len(test_x_2)):
        predicted_class = classifier.predict([test_x_2[i]])[0]
        true_class = test_y[i]
    ...
    print(f"Accuracy: {accuracy/len(test_x_2)}")




    test_x_input = [to_classifie[i] for i in range(len(to_classifie)) if i != col_index]

    predicted_class_2 = classifier.predict([test_x_input])[0]

    print(predicted_class_2)
    final = classifier.predict_proba([test_x_input])
    print(final[0])
