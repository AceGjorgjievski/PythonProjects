...
class Disks(Problem):
    def __init__(self,L,N,initial,goal=None):
        super(Disks, self).__init__(initial,goal)
        self.N = N
        self.L = L
    ...
    def successor(self, state):
        ...
        for i in range(length):
            if  i+1 < length and array[i+1] == 0:
                new_array = list(array)
                new_array[i+1] = array[i]
                new_array[i] = 0
                successors[f"D1: Disk {array[i]}"] = tuple(new_array)
            if  i+2 < length and array[i+2] == 0 and array[i+1] != 0:
                new_array = list(array)
                new_array[i+2] = array[i]
                new_array[i] = 0
                successors[f"D2: Disk {array[i]}"] = tuple(new_array)

            if i > 0 and array[i-1] == 0:
                new_array = list(array)
                new_array[i-1] = array[i]
                new_array[i] = 0
                successors[f"L1: Disk {array[i]}"] = tuple(new_array)
            if i > 1 and array[i-1] != 0 and array[i-2] == 0:
                new_array = list(array)
                new_array[i-2] = array[i]
                new_array[i] = 0
                successors[f"L2: Disk {array[i]}"] = tuple(new_array)
    ...
    def h(self, node):
        ...
        # for i in state:
        #     if i < self.L-self.N:
        #         sum += mhd(node.state.index(i),self.L-1)
        #     else:
        #         break
        # return sum
        ...
        for i,j in zip(node.state, self.goal):
            if i!=j:
                counter += 1
        return counter/2 #go delime so 2 bidejkji mozhe za 2 pozicii da se pomesti
...
if __name__ == "__main__":
    ...
    disk = Disks(L,N,tuple(disk),tuple(goal))
    result = astar_search(disk)
    ...
    #todo naopachki da vezhbam iterachija i potoa primena kaj rechnik
