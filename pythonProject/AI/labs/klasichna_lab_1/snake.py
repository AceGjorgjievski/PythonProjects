...
#                           0       6       [(),(),...]
def move_down(head_x, head_y, red_apples):
    if 0 < head_y < 10 and (head_x, head_y-1) not in red_apples:
        head_y -= 1
    return head_y

def move_up(head_x, head_y, red_apples):
    if 0 < head_y+1 < 10 and (head_x, head_y+1) not in red_apples:
        head_y += 1
    return head_y

def move_left(head_x, head_y, red_apples):
    if 0 < head_x < 10 and (head_x-1, head_y) not in red_apples:
        head_x -= 1
    return head_x

def move_right(head_x, head_y, red_apples):
    if 0 < head_x+1 < 10 and (head_x+1, head_y) not in red_apples:
        head_x += 1
    return head_x


def check_apples(head_x, head_y, apples):
    for (x,y) in apples:
        if (head_x, head_y) == (x, y):
            return True
    return False

def check_Collisions(head_x, head_y, body):
    for pair in body[1:]:
        if (head_x, head_y) == (pair[0], pair[1]):
            return True
    return False
...
class Snake(Problem):
    ...
    def successor(self, state):
        ...
        dir = state[2]
        body = state[1]
        head = state[0]# samo glava
        head_x, head_y = head[0], head[1] #xpos na glava, ypos na glava

        apples = state[3]


        #(snake_head,snake_body,nasoka,tuple(zeleni_jabolki)

        if dir == "jug":
            y_new = move_down(head_x, head_y, self.red_apples)
            if y_new != head_y:
                if check_Collisions(head_x, y_new, body) == False:
                    new_x = head_x
                    new_y = y_new + 1
                    if check_apples(head_x, y_new, apples) == True:
                        new_body = list(body)
                        new_body.insert(0, (new_x, new_y))

                        succ["ProdolzhiPravo"] = ((head_x, y_new), tuple(new_body), "jug",
                                                  tuple([a for a in apples if [head_x, y_new] != [a[0], a[1]]]))
                    else:
                        new_body = list(body)
                        ...
                        new_body.insert(0, (new_x, new_y))
                        succ["ProdolzhiPravo"] = ((head_x, y_new), tuple(new_body), "jug", tuple(apples))

            x_new = move_left(head_x, head_y, self.red_apples)
            if x_new != head_x:
                if check_Collisions(x_new, head_y, body) == False:
                    new_x = x_new + 1
                    new_y = head_y
                    if check_apples(x_new, head_y, apples) == True:
                        new_body = list(body)
                        new_body.insert(0, (new_x, new_y))

                        succ["SvrtiDesno"] = ((x_new, head_y), tuple(new_body), "zapad",
                                              tuple([a for a in apples if [x_new, head_y] != [a[0], a[1]]]))
                    else:
                        new_body = list(body)
                        new_body.pop()

                        new_body.insert(0, (new_x, new_y))
                        succ["SvrtiDesno"] = ((x_new, head_y), tuple(new_body), "zapad", tuple(apples))

            x_new = move_right(head_x, head_y, self.red_apples)
            if x_new != head_x:
                if check_Collisions(x_new, head_y, body) == False:
                    new_x = x_new - 1
                    new_y = head_y
                    if check_apples(x_new, head_y, apples) == True:
                        ...
                        succ["SvrtiLevo"] = ((x_new, head_y), tuple(new_body), "istok",
                                             tuple([a for a in apples if [x_new, head_y] != [a[0], a[1]]]))
                    else:
                        new_body = list(body)
                        new_body.pop()
                        ...
                        new_body.insert(0, (new_x, new_y))
                        succ["SvrtiLevo"] = ((x_new, head_y), tuple(new_body), "istok", tuple(apples))
            ...
            if x_new != head_x:
                if check_Collisions(x_new, head_y, body) == False:
                    ...
                    if check_apples(x_new, head_y, apples) == True:
                        ...
                        succ["ProdolzhiPravo"] = ((x_new, head_y), tuple(new_body), "zapad",
                                                  tuple([a for a in apples if [x_new, head_y] != [a[0], a[1]]]))
                    else:
                        new_body = list(body)
                        new_body.pop()
                        ...
                        new_body.insert(0, (new_x, new_y))
                        succ["ProdolzhiPravo"] = ((x_new, head_y), tuple(new_body), "zapad", tuple(apples))
            y_new = move_up(head_x, head_y, self.red_apples)
            if y_new != head_y:
                if check_Collisions(head_x, y_new, apples) == False:
                    new_x = head_x
                    new_y = y_new - 1
                    if check_apples(head_x, y_new, apples) == True:
                        new_body = list(body)
                        ...
                        succ["SvrtiDesno"] = ((head_x, y_new), tuple(new_body), "sever",
                                              tuple([a for a in apples if [head_x, y_new] != [a[0], a[1]]]))
                    else:
                        new_body = list(body)
                        new_body.pop()
                        ...
                        succ["SvrtiDesno"] = ((head_x, y_new), tuple(new_body), "sever", tuple(apples))
            y_new = move_down(head_x, head_y, self.red_apples)
            if y_new != head_y:
                if check_Collisions(head_x, y_new, apples) == False:
                    new_x = head_x
                    ...
                    if check_apples(head_x, y_new, apples) == True:
                        ...
                        succ["SvrtiLevo"] = ((head_x, y_new), tuple(new_body), "jug",
                                             tuple([a for a in apples if [head_x, y_new] != [a[0], a[1]]]))
                    else:
                        new_body = list(body)
                        new_body.pop()
                        ...
                        succ["SvrtiLevo"] = ((head_x, y_new), tuple(new_body), "jug", tuple(apples))
            ...
            if x_new != head_x:
                if check_Collisions(x_new, head_y, body) == False:
                    ...
                    if check_apples(x_new, head_y, apples) == True:
                        ...
                        succ["ProdolzhiPravo"] = ((x_new, head_y), tuple(new_body), "istok",
                                                  tuple([a for a in apples if [x_new, head_y] != [a[0], a[1]]]))
                    else:
                        new_body = list(body)
                        new_body.pop()
                        ...
                        new_body.insert(0, (new_x, new_y))
                        succ["ProdolzhiPravo"] = ((x_new, head_y), tuple(new_body), "istok", tuple(apples))
            y_new = move_down(head_x, head_y, self.red_apples)
            if y_new != head_y:
                if check_Collisions(head_x, y_new, body) == False:
                    new_x = head_x
                    ...
                    if check_apples(head_x, y_new, apples) == True:
                        ...
                        succ["SvrtiDesno"] = ((head_x, y_new), tuple(new_body), "jug",
                                              tuple([a for a in apples if [head_x, y_new] != [a[0], a[1]]]))
                    else:
                        new_body = list(body)
                        ...
                        new_body.insert(0, (new_x, new_y))
                        succ["SvrtiDesno"] = ((head_x, y_new), tuple(new_body), "jug", tuple(apples))
            y_new = move_up(head_x, head_y, self.red_apples)
            if y_new != head_y:
                if check_Collisions(head_x, y_new, body) == False:
                    new_x = head_x
                    ...
                    if check_apples(head_x, y_new, apples) == True:
                        ...
                        succ["SvrtiLevo"] = ((head_x, y_new), tuple(new_body), "sever",
                                             tuple([a for a in apples if [head_x, y_new] != [a[0], a[1]]]))
                    else:
                        new_body = list(body)
                        ...
                        new_body.insert(0, (new_x, new_y))
                        succ["SvrtiLevo"] = ((head_x, y_new), tuple(new_body), "sever", tuple(apples))
            ...
            if y_new != head_y:
                if check_Collisions(head_x, y_new, body) == False:
                    ...
                    if check_apples(head_x, y_new, apples) == True:
                        ...
                        succ["ProdolzhiPravo"] = ((head_x, y_new), tuple(new_body), "sever",
                                                  tuple([a for a in apples if [head_x, y_new] != [a[0], a[1]]]))
                    else:
                        new_body = list(body)
                        ...
                        new_body.insert(0, (new_x, new_y))
                        succ["ProdolzhiPravo"] = ((head_x, y_new), tuple(new_body), "sever", tuple(apples))
            x_new = move_right(head_x, head_y, self.red_apples)
            if x_new != head_x:
                if check_Collisions(x_new, head_y, body) == False:
                    new_x = x_new - 1
                    ...
                    if check_apples(x_new, head_y, apples) == True:
                        ...
                        succ["SvrtiDesno"] = ((x_new, head_y), tuple(new_body), "istok",
                                              tuple([a for a in apples if [x_new, head_y] != [a[0], a[1]]]))
                    else:
                        new_body = list(body)
                        new_body.pop()
                        ...
                        new_body.insert(0, (new_x, new_y))
                        succ["SvrtiDesno"] = ((x_new, head_y), tuple(new_body), "istok", tuple(apples))
            ...
            if x_new != head_x:
                if check_Collisions(x_new, head_y, body) == False:
                    ...
                    if check_apples(x_new, head_y, apples) == True:
                        ...
                        succ["SvrtiLevo"] = ((x_new, head_y), tuple(new_body), "zapad",
                                             tuple([a for a in apples if [x_new, head_y] != [a[0], a[1]]]))
                    else:
                        new_body = list(body)
                        new_body.pop()
                        ...
                        new_body.insert(0, (new_x, new_y))
                        succ["SvrtiLevo"] = ((x_new, head_y), tuple(new_body), "zapad", tuple(apples))
...
if __name__ == '__main__':
    ...
    # print(type(crveni_jabolki))
    # print(type(zeleni_jabolki))
    # print(type(snake_head))
    # print(type(snake_body))
    ...
    # snake_pos.append((0,1))
    # print(snake_pos)

    # var = False - working
    # for part in crveni_jabolki:
    #     for part2 in snake_pos:
    #         if part[0] == part2[0] and part[1] == part2[1]:
    #             var = True
    #
    # print(var)
    ...
    #     if headx == body_part[0] and heady == body_part[1]:
    #         print("Collision")
    #
    #
    #
