...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        x,y = state
        if state in walls or x < 0 or x > n-1 or y < 0 or y > n-1:
            return False
        return True
    ...
    def successor(self, state):
        ...
        if self.check_valid((x,y+1),self.walls,n) and y < n-1:
            successors["Gore 1"] = (x,y+1)
        if self.check_valid((x,y+2), self.walls,n) and y < n-2:
            successors["Gore 2"] = (x, y+2)
        if self.check_valid((x,y+3),self.walls,n) and y < n-3:
            successors["Gore 3"] = (x, y+3)

        if self.check_valid((x+1,y),self.walls,n) and x < n-1:
            successors["Desno 1"] = (x+1,y)
        if self.check_valid((x+2,y),self.walls,n) and x < n-2:
            successors["Desno 2"] = (x+2,y)
        if self.check_valid((x+3,y),self.walls,n) and x < n-3:
    ...
    def h(self, node):
        ...
        return self.mhd(px,py,gx,gy)/3

    def mhd(self, sx,sy,gx,gy):
